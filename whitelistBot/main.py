    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if (message.channel.id == self.data[str(message.guild.id)]['whitelist_channel']
            ...
            if self.validators[self.data[str(message.guild.id)]['blockchain']](message.content):
                ...
                await message.reply(
                    f"{message.author.display_name}, your wallet `{message.content}` has been validated and recorded. Prepare for depature to Nos Atomos.", mention_author=True)
                pioneer = message.guild.get_role(928733820235182090)
                await message.author.add_roles(pioneer)
